...
static void cdc_send_start(uint8_t busid,CDC_UART *cu)
{
	int length;
	...
	if(cu->ctx_rp==0 && cu->ctx_wp==2){
		...
		tbuf[0] = 0x02;
		tbuf[1] = 0x61;
		usbd_ep_start_write(busid,cu->in_ep.ep_addr, tbuf, length);
		cu->ctx_en = 1;
	}else{
		// 数据在txbuf两端. [........w         r........]
...
void cdc_recv_start(uint8_t busid,CDC_UART *cu)
{
	usbd_ep_start_read(busid,cu->out_ep.ep_addr, cu->usb_rxbuf, USB_PACKET_SIZE);
}
...
static void cdc_bulk_in_cb(uint8_t busid,uint8_t ep, uint32_t nbytes)
{
	...
	if(nbytes==USB_PACKET_SIZE){
		// Send ZLP
		usbd_ep_start_write(busid,cu->in_ep.ep_addr, NULL, 0);
		return;
	}
	...
	if(cu->ctx_rp+2 == cu->ctx_wp){
		...
		tbuf[0] = 0x02;
		tbuf[1] = 0x60;
		usbd_ep_start_write(busid,cu->in_ep.ep_addr, tbuf, 2);
		cu->ctx_en = 1;
	}else{
		//cdclog(3, cu->ctx_rp, cu->ctx_wp);
		cdc_send_start(busid,cu);
	}
}


void cdc_send_startup(uint8_t busid,CDC_UART *cu)
{
	uint8_t *tbuf = cu->usb_txbuf+cu->ctx_rp;
	tbuf[0] = 0x02;
	tbuf[1] = 0x60;
...
void uart_irq_handle(CDC_UART *cu)
{
	...
	if(cu->uart->STATR & 0x0040){
		...
		if(cu->crx_size){
			...
			if(cu->crx_size==0){
				usbd_ep_start_read(0,cu->out_ep.ep_addr, cu->usb_rxbuf, USB_PACKET_SIZE);
			}else{
				uart_send_start(cu);
			}
		}else{
...
void usbd_cdc_acm_set_line_coding(uint8_t busid,uint8_t intf, struct cdc_line_coding *line_coding)
{
	...
	DEBUG_PRINT("CDC set line_coding: %d %d-%d-%d\n",
...
extern int usbd_ftdi_vendor_handler(uint8_t busid,struct usb_setup_packet *setup, uint8_t **data, uint32_t *len);
void cdcuart_init(uint8_t busid,CDC_UART *cu)
{
	DEBUG_PRINT("\ncdcuart_init ...\n");//printk
	usbd_cdc_acm_init_intf(0,&cu->intf0);
	// cu->intf0.vendor_handler = usbd_ftdi_vendor_handler;
	// usbd_add_interface(0,&cu->intf0);
	usbd_add_interface(0,usbd_cdc_acm_init_intf(0,&cu->intf1));

	cu->out_ep.ep_cb = cdc_bulk_out_cb;
	usbd_add_endpoint(0,&cu->out_ep);
	cu->in_ep.ep_cb = cdc_bulk_in_cb;
	usbd_add_endpoint(0,&cu->in_ep);

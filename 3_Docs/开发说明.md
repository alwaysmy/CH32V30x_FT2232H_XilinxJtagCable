...
做好识别之后，Vivado能认出来模拟的调试线，但是连接不上设备，经过一顿抓包，发现vivado在初始的时候会读取FT设备的端口模式，实际上就在MCU里面就是读取IO方向寄存器，低4位设置为JTAG对应的模式，这个肯定没问题。比较神秘的是，如果把高4位原模原样的传回去（实际上就是0），他就会跳循环，不会进入下面的发jtag命令检测环节；原始的FT232设备读出来的值高4位是d(1b'1101），但是高四位压根没接线，按理说也没配置，所以我不理解这里为啥vivido一定要是这样。所以我选择复制粘贴了0xd0加到原始值上，这个问题就解决了。

不过也许是EEPROM里面有字段进行了配置，由于我只是模拟了EEPROM，并没有模拟EEPROM对应的所有配置功能？也许是原来最开始用FT232的 Digilent板子上高位用到其他功能？所以这里是个屎山？



最神秘的是，由于我一开始写错了，恰巧返回值变成了0xFF，vivado也能用。。。
...
[将FT4232H和program_ftdi实用程序与 2 个 JTAG 一起使用](https://adaptivesupport.amd.com/s/question/0D54U00006yXEODSA4/using-ft4232h-and-programftdi-utility-with-2-jtags?language=en_US)
